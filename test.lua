...
local c = ssdb_client.newclient({timeout=10000})
...
function test_queue()
    local q_ = uk()
    local q = q_ .. 'queue'
    local res, err = c:qpush(q, 'a')
    assert(res == 1 and not err)
    local res, err = c:qpush_back(q, 'b')
    assert(res == 2 and not err)
    local res, err = c:qpush_front(q, 'c')
    assert(res == 3 and not err)
    local res, err = c:qsize(q)
    assert(res == 3 and not err)
    local res, err = c:qfront(q)
    assert(res == 'c' and not err)
    local res, err = c:qback(q)
    assert(res == 'b' and not err)
    local res, err = c:qget(q, 1)
    assert(res  == 'a' and not err)
    local res, err = c:qslice(q, 0, 1)
    assert(table.eql(res, {'c', 'a'}) and not err)
    local res, err = c:qlist(q_, q, -1)
    assert(table.eql(res, {q}) and not err)
    local res, err = c:qrlist(q, q_, -1)
    assert(table.eql(res, {}) and not err)
    local res, err = c:qpop(q)
    assert(res == 'c' and not err)
    local res, err = c:qpop_front(q)
    assert(res == 'a' and not err)
    local res, err = c:qpop_back(q)
    assert(res == 'b' and not err)
    local res, err = c:qclear(q)
    assert(res == 0 and not err)
end
